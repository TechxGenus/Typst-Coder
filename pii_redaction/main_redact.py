...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(f"pii_redaction/logs/pii-{args.dataset_name.split('/')[-1]}.log"),
    ...
    if len(ds_checks) == 0:
        ...
        if args.save_mode_checks == "hub":
            ...
            ds_checks = ds_checks.to_list()
            with open(args.save_path_disk + "_checks.json", "w") as f:
                json.dump(ds_checks, f, indent=4)
    ...
    if args.save_mode == "hub":
        ...
        ds_pii = ds_pii.to_list()
        with open(args.save_path_disk + ".json", "w") as f:
            json.dump(ds_pii, f, indent=4)
